...
let turn = "red"; // Lượt đi: đỏ trước, đen sau
let moveHistory = [];
...
let redTime = 2700; // 45 phút = 2700 giây
let blackTime = 2700; // 45 phút = 2700 giây
...
function startTimer() {
    ...
    timer = setInterval(() => {
        if (turn === "red") {
            ...
            if (redTime <= 0) {
                stopTimer();
                alert("Do Thời Gian Bạn Ra Nước Cờ Chậm Và Xuất Quân Không Kịp Thời Nên Bạn Bị Xử Thua!...kkk...Làm Ván Mới Thôi Nè!");
                endGame("Đen");
            }
        } else {
            blackTime--;
...
// Sự kiện nút "Vào Chơi Với Bot AI"
document.getElementById("playBotBtn").addEventListener("click", () => {
    document.getElementById("playBotModal").style.display = "flex";
});

// Sự kiện nút "Thôi" trong modal chế độ chơi với Bot AI
document.getElementById("playBotNoBtn").addEventListener("click", () => {
    document.getElementById("playBotModal").style.display = "none";
});

// Sự kiện nút "OK" trong modal chế độ chơi với Bot AI
document.getElementById("playBotOkBtn").addEventListener("click", () => {
    window.location.href = "vs/indexvs.html"; // Chuyển về trang indexvs.html trong thư mục vs
});
